...
class DataGenerator {
    static func generateViewModels1(collection: Bool = false) -> TableList {
        let list = TableList()
        ...
        for i in 0..<3 {
            ...
            for j in 0..<5 {
                if collection {
                    section.append(row: TableRow<JustCollectionCell>.init(id: "\(j)_\(i)", viewModel: JustCollectionCellVM(title: "\(j) in \(i)")))
                } else {
    ...
    static func generateViewModels2(collection: Bool = false) -> TableList {
        let list = generateViewModels1(collection: collection)
